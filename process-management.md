...
因为这个函数式使用嵌入汇编来实现的, 所以有点难懂, 不过主要过程就是通过调用 `_clone()`函数来创建一个新的进程, 而创建进程是通过传入 `CLONE_VM` 标志来指定进程借用其他进程的虚拟内存空间结构.
	...
	特别说明一下：`d0` 局部变量的作用是为了在创建内核线程时保证 `struct pt_regs` 结构的完整，这是因为创建内核线程是在内核态进行的，所以在内核态调用系统调用是不会压入 `ss` 和 `esp` 寄存器的，这样就会导致系统调用的 `struct pt_regs` 参数信息不完整，所以 `kernel_thread()` 函数定义了一个 `d0` 局部变量是为了补充没压栈的 `ss` 和 `esp` 的。
