...
*   由于 `Director` 服务器与 `Real-Server` 服务器在同一局域网中，所以通过数据包的目标 MAC 地址可以找到对应的 `Real-Server` 服务器，而 `Real-Server` 服务器接收到数据包后，由于设置了回环设备的 IP 地址为 VIP 地址，所以会对数据包进行处理（这个步骤很重要，因为如果不设置 VIP 为回环设备的 IP 地址，那么 `Real-Server` 服务器将会丢弃这个数据包）。
